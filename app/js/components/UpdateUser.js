...
class UpdateUser extends Component {
  ...
  _handleClick = async () => {
    ...
    const editAccount = DTwitter.methods.editAccount(web3.utils.keccak256(user.username), description, contact, amount, hash);


    // get a gas estimate for the transaction with the input username
    // and description
    const gasEstimate = await editAccount.estimateGas({ from: web3.eth.defaultAccount, gas: 1000000 });
  ...
  render() {
    const { isLoading, error, formState, formUpdated, description, contact, amount, picture } = this.state;
